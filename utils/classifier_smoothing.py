...
class ClassifierSmoothing(object):
    ...
    def __init__(self, base_classifier: tlx.nn.Module, num_classes: int, sigma: float):
        """
    ...
    def certify(self, z, n0: int, n: int, alpha: float, sampling_batch_size: int = 1000) -> Tuple[int, float]:
    ...
    def predict(self, z, n: int, alpha: float, sampling_batch_size: int) -> int:
    ...
    def _counts(self, z, num: int, sampling_batch_size: int) -> np.ndarray:
        ...
        with context_manager:
            ...
            z_repeated = tlx.ops.tile(z, [min(sampling_batch_size, num), 1])
            while num > 0:
                cur_num_copies = min(sampling_batch_size, num)
                num -= cur_num_copies

                if cur_num_copies != z_repeated.shape[0]:
                    z_repeated = tlx.ops.tile(z, [cur_num_copies, 1])

                assert z_repeated.shape[0] == z.shape[0] * cur_num_copies and z_repeated.shape[1] == z.shape[1]
                noise = tlx.ops.random_normal(z_repeated.shape, mean=0.0, stddev=self.sigma)

                logits = self.base_classifier(z_repeated + noise)
                predictions = (logits > 0).astype(tlx.int32)
